...
Future<void> main(List<String> args) async {
  ...
  switch (command) {
    ...
    default:
      ...
      print('Available commands: gradle-patch, clean-android, env-check, android-build');
  }
}
...
Future<void> _cleanAndroid() async {
  ...
  if (result.exitCode != 0) {
    throw Exception('clean_android.sh failed with exit code ${result.exitCode}');
  }
}
...
flutter.versionCode=1''';
  ...
  for (final line in debugLines.skip(debugLines.length > 20 ? debugLines.length - 20 : 0)) {
  ...
  for (final line in releaseLines.skip(releaseLines.length > 20 ? releaseLines.length - 20 : 0)) {
    if (line.trim().isNotEmpty) print(line);
  }
