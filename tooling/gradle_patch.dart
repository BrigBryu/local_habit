...
Future<void> patchIsarGradleFile() async {
  try {
    ...
    final isarDirs = pubCacheDirectory
        .listSync()
        .where((entity) =>
            entity is Directory && entity.path.contains('isar_flutter_libs'))
        .cast<Directory>()
        .toList();
    ...
    for (final isarDir in isarDirs) {
      final gradleFile =
          File(path.join(isarDir.path, 'android', 'build.gradle'));
  ...
  try {
    ...
    for (final isarDir in isarDirs) {
      final manifestFile = File(path.join(
          isarDir.path, 'android', 'src', 'main', 'AndroidManifest.xml'));
      ...
      final patchedContent =
          content.replaceAll(RegExp(r'\s*package="[^"]*"'), '');
