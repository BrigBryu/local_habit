...
class SyncQueueSimulator {
  ...
  Future<void> runSimulation() async {
    ...
    try {
      ...
      _logger.e('❌ SyncQueue simulation failed', error: e, stackTrace: stackTrace);
    } finally {
  ...
  Future<void> _initializeRepositories() async {
    ...
    await _localRepo.setCurrentUserId('sim_user_${DateTime.now().millisecondsSinceEpoch}');
  ...
  Future<void> _simulateOnlinePhase() async {
    ...
    if (initialPendingCount == 0) {
      _logger.w('⚠️  No pending operations found - simulation may not have worked correctly');
      return;
    }
    ...
    while (checkCount < maxChecks) {
      ...
      if (checkCount % 5 == 0) {
        ...
        _logger.i('📈 Progress: $processed/$initialPendingCount operations synced');
      }
    }
    ...
    if (finalPendingCount == 0) {
      ...
      _logger.w('⚠️  WARNING: $finalPendingCount operations still pending after 30 seconds');
    }
  ...
  void _mockNetworkFailure(bool shouldFail) {
    if (shouldFail) {
      _logger.w('🔌 NETWORK: Simulating offline mode (operations will be queued)');
