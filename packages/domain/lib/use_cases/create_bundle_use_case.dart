...
class CreateBundleUseCase {
  ...
  static void _validateChildHabits(List<String> childHabitIds, List<Habit> existingHabits) {
    ...
    for (final childId in childHabitIds) {
      final childHabit = existingHabits.where((h) => h.id == childId).firstOrNull;
      ...
      if (childHabit.isInBundle) {
        throw ArgumentError('Habit "${childHabit.name}" is already in a bundle');
      }
    }
  }
}